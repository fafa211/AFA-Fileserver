...
abstract class Auth {
	...
	public static function instance()
	{
		if ( ! isset(Auth::$_instance))
		{
			...
			$config = F::config('auth');
			

			if ( ! $type = $config['driver'])
			{
				$type = 'file';
			}
			...
			//载入驱动文件
			$file = F::find_file('driver', 'auth'.DIRECTORY_SEPARATOR.$type);
			include_once $file;
	...
	public function hash($str)
	{
		if ( ! $this->_config['hash_key'])
			throw new Exception('A valid hash key must be set in your auth config.', E_ERROR);
